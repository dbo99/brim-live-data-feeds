...
- **Publication time:** Version 1 serializes `publication_time_utc: null`. The
  builder cannot know the later Git commit, push, or Pages deployment time;
...
- **QA/failure policy:** All 41 inventories, exact record identity, range
  ...
  capped numeric `Retry-After`. Retrieval-time-only changes are
