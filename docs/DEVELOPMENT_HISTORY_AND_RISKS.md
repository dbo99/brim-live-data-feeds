...
| P1 | `main` had no observed branch protection or ruleset at the baseline, while scheduled writers intentionally publish ordinary non-force commits directly to it. | The branch can be deleted or force-pushed, rewriting or removing the official Pages history; an overbroad protection rule could instead stop all generated-data publication. | Add a narrow `main` ruleset that restricts deletion and blocks force pushes but leaves ordinary updates unrestricted. Test any future update, pull-request, check, signature or deployment requirement with the exact maintainer and Actions identities before activation. |
...
| P2 | The streamflow workflow injects `API_USGS_PAT`, but its builder does not read that variable. | A repository secret is exposed to a job that does not need it, increasing credential exposure without adding functionality. | Remove the streamflow secret injection in a focused workflow change and retain it only where the upstream client actually consumes it. |
