...
#
# Dry run is the default. Set BRIM_WSL_PUBLISH=true only from an authorized
# main-branch workflow run after the complete candidate bundle validates.
...
wsl_build <- function() {
  ...
  has_prior_canonical <- file.exists(prior_manifest_path)
  entries <- wsl_copy_retained_cycles(
    stage_root, entries, discovery$cycle_time, config, canonical_root,
    manifest_name = manifest_name, now = now
  )
  manifest <- wsl_manifest(discovery$cycle_time, entries, now, config)
  ...
  wsl_require_current_bootstrap(manifest, now, config, has_prior_canonical)

  destination_root <- if (publish) canonical_root else output_root
  promotion <- wsl_promote_bundle(stage_root, destination_root, manifest_name, config, now = now)
