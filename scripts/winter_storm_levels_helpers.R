...
WSL_S2_DENSIFY_GRID_CELLS <- 2
WSL_S2_CHAIKIN_PASSES <- 2L
WSL_S2_SIMPLIFY_GRID_CELLS <- 0.25
...
wsl_coordinates_to_feature <- function(feature, coordinates) {
  feature$geometry$coordinates <- unname(split(coordinates, row(coordinates)))
  feature
}

wsl_is_closed_coordinates <- function(coordinates) {
  nrow(coordinates) >= 4L &&
    isTRUE(all(coordinates[1L, ] == coordinates[nrow(coordinates), ]))
}

wsl_source_grid_spacing_m <- function(raster) {
  resolution <- abs(as.numeric(terra::res(raster)))
  if (length(resolution) != 2L || any(!is.finite(resolution)) || any(resolution <= 0)) {
    stop("SNOWLVL raster has invalid grid resolution.")
  }
  if (!isTRUE(terra::is.lonlat(raster))) return(mean(resolution))

  extent <- terra::ext(raster)
  center <- c((extent$xmin + extent$xmax) / 2, (extent$ymin + extent$ymax) / 2)
  samples <- rbind(center, center + c(resolution[1L], 0), center + c(0, resolution[2L]))
  points <- sf::st_sfc(lapply(seq_len(nrow(samples)), function(index) {
    sf::st_point(samples[index, ])
  }), crs = terra::crs(raster))
  projected <- sf::st_transform(points, WSL_CARTOGRAPHIC_CRS)
  spacing <- vapply(2:3, function(index) {
    as.numeric(sf::st_distance(projected[1L], projected[index]))
  }, numeric(1))
  spacing <- mean(spacing)
  if (!is.finite(spacing) || spacing <= 0) stop("Could not derive metric SNOWLVL grid spacing.")
  spacing
}
...
wsl_component_profile <- function(features, grid_spacing_m, config) {
  ...
  if (length(grid_spacing_m) != 1L || !is.finite(grid_spacing_m) || grid_spacing_m <= 0) {
    stop("Cartographic processing requires one positive finite grid spacing.")
  }
  domain_boundary <- sf::st_boundary(sf::st_transform(wsl_bbox_polygon(
    config$west, config$east, config$south, config$north
  ), WSL_CARTOGRAPHIC_CRS))
  rows <- lapply(seq_along(features), function(index) {
    coordinates <- wsl_feature_coordinates(features[[index]])
    if (nrow(coordinates) < 2L || any(!is.finite(coordinates)) || any(rowSums(abs(
      coordinates[-1L, , drop = FALSE] - coordinates[-nrow(coordinates), , drop = FALSE]
    )) == 0)) {
      wsl_stop("validation_failed", "Invalid input contour component: ", features[[index]]$id)
    }
    closed <- wsl_is_closed_coordinates(coordinates)
    line <- sf::st_transform(
      wsl_geometry_from_coordinates(coordinates, 4326), WSL_CARTOGRAPHIC_CRS
    )
    if (sf::st_is_empty(line) || !isTRUE(sf::st_is_valid(line)) ||
        !isTRUE(sf::st_is_simple(line)) || as.numeric(sf::st_length(line)) <= 0) {
      wsl_stop("validation_failed", "Invalid input contour linework: ", features[[index]]$id)
    }
    projected_coordinates <- sf::st_coordinates(line)[, 1:2, drop = FALSE]
    bounds <- sf::st_bbox(line)
    area_km2 <- NA_real_
    if (closed) {
      polygon <- sf::st_sfc(sf::st_polygon(list(projected_coordinates)), crs = WSL_CARTOGRAPHIC_CRS)
      area_km2 <- as.numeric(sf::st_area(sf::st_make_valid(polygon))) / 1e6
    }
    data.frame(
      index = index,
      feature_id = as.character(features[[index]]$id),
      closed = closed,
      length_m = as.numeric(sf::st_length(line)),
      enclosed_area_km2 = area_km2,
      max_projected_span_m = max(
        bounds[["xmax"]] - bounds[["xmin"]],
        bounds[["ymax"]] - bounds[["ymin"]]
      ),
      boundary_distance_m = as.numeric(sf::st_distance(line, domain_boundary)),
      vertices = nrow(coordinates),
      stringsAsFactors = FALSE
    )
  })
  profile <- do.call(rbind, rows)
  profile$remove <- with(profile,
    (closed & enclosed_area_km2 < grid_spacing_m ^ 2 / 1e6 &
       max_projected_span_m < 2 * grid_spacing_m) |
      (!closed & length_m < 2 * grid_spacing_m &
         boundary_distance_m > grid_spacing_m)
  )
  profile
}

wsl_filter_components_b <- function(features, grid_spacing_m, config) {
  profile <- wsl_component_profile(features, grid_spacing_m, config)
  keep <- !profile$remove
  list(
    features = features[keep],
    profile = profile,
    diagnostics = list(
      components_before = nrow(profile),
      components_after = sum(keep),
      closed_removed = sum(profile$remove & profile$closed),
      open_removed = sum(profile$remove & !profile$closed)
    )
  )
}

wsl_segmentize_coordinates <- function(coordinates, maximum_spacing_m) {
  if (nrow(coordinates) < 2L) return(coordinates)
  result <- coordinates[1L, , drop = FALSE]
  for (index in seq_len(nrow(coordinates) - 1L)) {
    start <- coordinates[index, ]
    finish <- coordinates[index + 1L, ]
    distance <- sqrt(sum((finish - start) ^ 2))
    pieces <- max(1L, ceiling(distance / maximum_spacing_m))
    fractions <- seq_len(pieces) / pieces
    additions <- t(vapply(fractions, function(fraction) {
      start + fraction * (finish - start)
    }, numeric(2)))
    result <- rbind(result, additions)
  }
  wsl_remove_consecutive_duplicates(result)
}

wsl_chaikin_once <- function(coordinates, closed) {
  if (closed) {
    unique_coordinates <- coordinates[-nrow(coordinates), , drop = FALSE]
    count <- nrow(unique_coordinates)
    if (count < 3L) stop("Closed contour requires at least three unique coordinates.")
    result <- matrix(numeric(), ncol = 2L)
    for (index in seq_len(count)) {
      next_index <- if (index == count) 1L else index + 1L
      start <- unique_coordinates[index, ]
      finish <- unique_coordinates[next_index, ]
      result <- rbind(
        result,
        0.75 * start + 0.25 * finish,
        0.25 * start + 0.75 * finish
      )
    }
    return(rbind(result, result[1L, ]))
  }

  if (nrow(coordinates) < 2L) stop("Open contour requires at least two coordinates.")
  result <- coordinates[1L, , drop = FALSE]
  for (index in seq_len(nrow(coordinates) - 1L)) {
    start <- coordinates[index, ]
    finish <- coordinates[index + 1L, ]
  ...
  rbind(result, coordinates[nrow(coordinates), , drop = FALSE])
}

wsl_chaikin <- function(coordinates, closed, passes) {
  passes <- as.integer(passes)
  if (length(passes) != 1L || is.na(passes) || passes < 0L) {
    stop("Chaikin pass count must be a nonnegative integer.")
  }
  result <- coordinates
  if (passes == 0L) return(result)
  for (pass in seq_len(passes)) result <- wsl_chaikin_once(result, closed)
  result
}

wsl_finalize_s2_feature <- function(feature, projected_line, coordinate_digits) {
  source_coordinates <- wsl_feature_coordinates(feature)
  originally_closed <- wsl_is_closed_coordinates(source_coordinates)
  geographic <- sf::st_transform(projected_line, 4326)
  result <- round(sf::st_coordinates(geographic)[, 1:2, drop = FALSE], coordinate_digits)
  result <- wsl_remove_consecutive_duplicates(result)

  if (originally_closed) {
    if (nrow(result) >= 3L && !all(result[1L, ] == result[nrow(result), ])) {
      result <- rbind(result, result[1L, ])
    }
  } else if (nrow(result) >= 2L) {
    result[1L, ] <- round(source_coordinates[1L, ], coordinate_digits)
    result[nrow(result), ] <- round(source_coordinates[nrow(source_coordinates), ], coordinate_digits)
    result <- wsl_remove_consecutive_duplicates(result)
  }

  if (nrow(result) < 2L || (originally_closed && nrow(result) < 4L)) {
    wsl_stop("validation_failed", "S2 processing collapsed retained component: ", feature$id)
  }
  final_line <- wsl_geometry_from_coordinates(result, 4326)
  final_length <- as.numeric(sf::st_length(sf::st_transform(final_line, WSL_CARTOGRAPHIC_CRS)))
  if (!is.finite(final_length) || final_length <= 0 || sf::st_is_empty(final_line) ||
      !isTRUE(sf::st_is_valid(final_line)) || !isTRUE(sf::st_is_simple(final_line))) {
    wsl_stop("validation_failed", "S2 processing produced invalid linework: ", feature$id)
  }
  if (wsl_is_closed_coordinates(result) != originally_closed) {
    wsl_stop("validation_failed", "S2 processing changed component closure: ", feature$id)
  }
  if (!originally_closed &&
      (!all(result[1L, ] == round(source_coordinates[1L, ], coordinate_digits)) ||
       !all(result[nrow(result), ] == round(source_coordinates[nrow(source_coordinates), ], coordinate_digits)))) {
    wsl_stop("validation_failed", "S2 processing moved an open endpoint: ", feature$id)
  }

  original_semantics <- feature$properties[setdiff(names(feature$properties), "length_m")]
  feature <- wsl_coordinates_to_feature(feature, result)
  feature$properties$length_m <- round(final_length, 1)
  if (!identical(
    feature$properties[setdiff(names(feature$properties), "length_m")],
    original_semantics
  )) {
    wsl_stop("validation_failed", "S2 processing changed scientific properties: ", feature$id)
  }
  feature
}

wsl_smooth_component_s2 <- function(feature, grid_spacing_m, coordinate_digits) {
  source_coordinates <- wsl_feature_coordinates(feature)
  closed <- wsl_is_closed_coordinates(source_coordinates)
  projected <- sf::st_coordinates(sf::st_transform(
    wsl_geometry_from_coordinates(source_coordinates, 4326), WSL_CARTOGRAPHIC_CRS
  ))[, 1:2, drop = FALSE]
  segmentized <- wsl_segmentize_coordinates(
    projected, WSL_S2_DENSIFY_GRID_CELLS * grid_spacing_m
  )
  smoothed <- wsl_chaikin(segmentized, closed, WSL_S2_CHAIKIN_PASSES)
  generalized <- sf::st_simplify(
    wsl_geometry_from_coordinates(smoothed, WSL_CARTOGRAPHIC_CRS),
    dTolerance = WSL_S2_SIMPLIFY_GRID_CELLS * grid_spacing_m,
    preserveTopology = TRUE
  )
  wsl_finalize_s2_feature(feature, generalized, coordinate_digits)
}

wsl_apply_s2_cartography <- function(geojson, grid_spacing_m, config) {
  if (!identical(geojson$type, "FeatureCollection") || !length(geojson$features)) {
    stop("S2 cartographic processing requires a nonempty GeoJSON FeatureCollection.")
  }
  filtered <- wsl_filter_components_b(geojson$features, grid_spacing_m, config)
  if (!length(filtered$features)) {
    wsl_stop("validation_failed", "The fixed B filter removed every contour component.")
  }
  features <- lapply(filtered$features, wsl_smooth_component_s2,
                     grid_spacing_m = grid_spacing_m,
                     coordinate_digits = config$coordinate_digits)
  ids <- vapply(features, `[[`, character(1), "id")
  if (anyDuplicated(ids)) wsl_stop("validation_failed", "S2 output contains duplicate feature IDs.")
  coordinates <- do.call(rbind, lapply(features, wsl_feature_coordinates))
  geojson$features <- features
  geojson$bbox <- unname(c(
    min(coordinates[, 1L]), min(coordinates[, 2L]),
    max(coordinates[, 1L]), max(coordinates[, 2L])
  ))
  filtered$diagnostics$vertices <- sum(vapply(
    features, function(feature) nrow(wsl_feature_coordinates(feature)), integer(1)
  ))
  list(geojson = geojson, diagnostics = filtered$diagnostics, profile = filtered$profile)
}
